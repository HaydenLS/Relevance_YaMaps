...
Скачать датасет [link](https://drive.google.com/drive/folders/1iwfLcQmOqnrcXZlMOPRclY1OIKQ3-Bqa?usp=sharing)
---
...
Было проведено множество экспериментов. Основные выводы:
* Модель плохо понимает метку `0.1` - она является весьма субъективной, граница между 0.1 и 1.0 субъективна.
* Удаление метки 0.1 и переход к бинарной классификации улучшили устойчивость предсказаний.

Итог:
```
Accuracy ≈ 0.73
Macro F1 ≈ Weighted F1 ≈ 0.72
```

Модель хорошо находит релевантные организации (высокий recall для 1.0), но сохраняется умеренное количество ложноположительных предсказаний.
